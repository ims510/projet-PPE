...
do
	...
	WORDCOUNT=$(cat ../dumps-text/dump_ru_$N.txt | egrep -o "[Вв]ойн((ам?и?х?)|(ы)|(у)|(ой)|(е))?"| wc -w)
	cat ../dumps-text/dump_ru_$N.txt | sed '/^$/d'| egrep -C 1 "[Вв]ойн((ам?и?х?)|(ы)|(у)|(ой)|(е))?" > ../contexte/contexte_ru_$N.txt
	./concordancier_russe.sh ../dumps-text/dump_ru_$N.txt ../tableaux/concordances_russe_$N.html
	...
	TABLEROW="<tr><td>$N</td><td>$HTTPCODE</td><td><a href=${line}>${line}</a></td><td>$ENCODING</td><td><a href="../aspirations/url_ru_$N.html">aspiration</a></td><td><a href="../dumps-text/dump_ru_$N.txt">dump</a></td><td>$WORDCOUNT</td><td><a href="../contexte/contexte_ru_$N.txt">contexte</a></td>""<td><a href="../tableaux/concordances_russe_$N.html">concordances</a></td></tr>\n"
	TABLE+="$TABLEROW\n"
	N=$(expr $N + 1)
done < "$FILE_PATH_IN"
